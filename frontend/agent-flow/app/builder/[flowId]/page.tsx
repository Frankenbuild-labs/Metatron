...
import { MessageSquare, BrainCircuit, Puzzle, ArrowRightCircle, DownloadCloud, PanelLeftOpen, PanelRightOpen, PanelLeftClose, PanelRightClose, Trash2, Group, Share2, Upload, Loader2, Save, X, Calendar } from 'lucide-react';
...
const modelOptions = {
  ...
  coordinator: ['metatron-coordinator'],
};
...
const sidebarNodeTypes = [
  ...
  { type: 'composio', label: 'Composio Tool', icon: <Puzzle size={18} />, defaultData: { label: 'Composio Action', apiKey: '', toolAction: '' }, category: 'Tools' },
...
const allSidebarItems = [
  ...
  // Create template sidebar items
  const templateSidebarItems = savedTemplates.map(template => ({
    key: template.id,
    label: template.name,
    icon: <Save size={18} />,
    description: template.description || `${template.nodes.length} nodes, ${template.edges.length} connections`,
  ...
  const onNodeDataChange = useCallback(
    (id: string, newData: Partial<InputNodeData | LLMNodeData | ComposioNodeData | AgentNodeData | CoordinatorNodeData>) => {
      setNodes((nds) =>
        nds.map((node) =>
  ...
  const nodeTypes: NodeTypes = useMemo(() => ({
    ...
    coordinator: (props) => <CoordinatorNode {...props} data={{ ...props.data, onNodeDataChange: onNodeDataChange as any}} onCopyApiKeyToAllCoordinators={(apiKey) => setNodes(nds => nds.map(n => n.type === 'coordinator' ? { ...n, data: { ...n.data, apiKey } } : n))} />,
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col flex-grow">
                        ...
                        {item.dragType === 'template' && (
                          <div className="text-xs text-[#fff5f5]/50 mt-1 flex items-center gap-2">
                            <Calendar size={10} />
